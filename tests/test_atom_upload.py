...
class TestAtomUpload(unittest.TestCase):
    def test_rsync_fail(self):
        ...
        with mock.patch("subprocess.check_output", side_effect=effect):
            self.assertRaises(
                subprocess.CalledProcessError, atom_upload.rsync, RSYNC_TARGET, DIP_PATH
            )
    ...
    @vcr.use_cassette("fixtures/vcr_cassettes/test_atom_upload_deposit_fail.yaml")
    ...
    def test_deposit_success(self):
        ret = atom_upload.deposit(
            ATOM_URL, ATOM_EMAIL, ATOM_PASSWORD, ATOM_SLUG, DIP_PATH
        )
