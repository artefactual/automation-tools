...
def main(source_sip, target_dir, csv_delimiter, prefix=None, metadata_only=False):
    ...
    print(
        "\033[92m{}: {}\033[00m".format(
            "submissionDocumentation should be available at", sdoc_dir_dst
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Split a single large transfer into several"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=",",
        help="Delimiter of the CSV metadata file.",
    )
    parser.add_argument(
        "--prefix",
        type=str,
        default=None,
        help="Prefix of the resulting split transfers.",
    )
    parser.add_argument(
        "--metadata-only",
        action="store_true",
        help="Only update the metadata.csv; do not move files",
    )
